...
from app import app
import json
from app.utils.helper import answer1_to_post, answer2_to_post, answer3_to_post
...
class TestAnswer(TestCase):
    ...
    def post_answer(self):
        response = self.client.post(path='api/v1/answer', data=json.dumps(answer1_to_post),
    ...
    def test_create_question(self):
         with self.client:
             ...
             response = self.client.post('api/v1/answer', data=json.dumps(answer1_to_post),
    ...
    def test_question_with_no_title(self):
         with self.client:
             ...
             response = self.client.post('api/v1/answer', data=json.dumps(answer2_to_post),
                                         headers = {'content-type': 'application' '/json'})
             self.assertEqual(response.status_code, 201)    
    ...
    def test_edit_answer(self):
        ...
        with self.client:
            self.post_answer()
            response = self.client.put('api/v1/answer/1', data=json.dumps(answer1_to_post),
                                         headers={'Content-Type': 'application' '/json'})
